    ...
    if ispay[0] == "yes":
        for i in range(int("".join(event.text.split(maxsplit=2)[2:]).split(" ", 2)[0])):
            if ispay[0] == 'yes':
                break
            chat = event.chat_id
            await jepthon.send_message(chat, 'فلوسي')
            await asyncio.sleep(0.5)
            masg = await jepthon.get_messages(chat, limit=1)
            masg = masg[0].message
            masg = ("".join(masg.split(maxsplit=2)[2:])).split(" ", 2)
            msg = masg[0]
            if int(msg) > 500000000:
                await jepthon.send_message(chat, f"استثمار {msg}")
                await asyncio.sleep(10)
                mssag2 = await jepthon.get_messages(chat, limit=1)
